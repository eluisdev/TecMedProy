...
export default function Custodios() {
    ...
    const { eliminarCustodio } = useProyect();
    const navigate = useNavigate();

    const token = localStorage.getItem('AUTH_TOKEN')

    const fetcher = () => clienteAxios('/api/interesteds', {
        headers: {
    ...
    const handleDelete = (nombre, id) => {
        Swal.fire({
            title: nombre,
            text: "Esta seguro de querer eliminar el custodio",
            icon: "warning",
    ...
    return (
        <>
            <div className="flex justify-center mt-3">
                <Link
                    to={'/administrador/caja-chica/custodio/nuevo'}
                    ...
                    <tbody>
                        {
                            custodios?.map(custodio => (
                                <tr className="bg-white border-b" key={custodio.id}>
                                    <th scope="row" className="p-2 font-medium">
                                        {custodio.nombreCompleto}
                                    </th>
                                    <td className="p-2">
                                        <button className="bg-yellow-500 hover:bg-yellow-700 w-8 h-8 rounded-full sm:mx-1" onClick={() => {
                                            navigate(`/administrador/caja-chica/custodio/editar/${custodio.id}`)
                                        }}>
                                        ...
                                        <button className="bg-red-500 hover:bg-red-700 w-8 h-8 rounded-full" onClick={() => {
                                            handleDelete(custodio.nombreCompleto, custodio.id)
                                        }}>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-white mx-auto">
