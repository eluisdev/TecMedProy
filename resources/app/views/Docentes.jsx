...
export default function Docentes() {
    ...
    const { eliminarDocente, filtrado, changeView } = useProyect();
    ...
    const handleDelete = (nombre, id) => {
        Swal.fire({
            title: nombre,
            text: "Esta seguro de querer eliminar al docente",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                const mostrarRespuesta = async () => {
                    const respuesta = await eliminarDocente(id);
        ...
        <>
            ...
            <div className="relative overflow-x-auto pt-5">
                <table className="w-3/4 text-sm text-center mx-auto">
                    ...
                    <tbody>
                        {
                            filteredItems.map(docente => (
                                <tr className="bg-white border-b" key={docente.id}>
                                    <th scope="row" className="p-2 font-medium">
                                        {docente.nombreCompleto}
                                    </th>
                                    {/* <td className="p-2">
                                        Aportes de la Carrera.
                                    </td> */}
                                    <td className="p-2">
                                        <button className="bg-yellow-500 hover:bg-yellow-700 w-8 h-8 rounded-full sm:mx-1" onClick={() => navigate(`/administrador/docentes/editar/${docente.id}`)}>
                                        ...
                                        <button className="bg-red-500 hover:bg-red-700 w-8 h-8 rounded-full" onClick={() => handleDelete(docente.nombreCompleto, docente.id)}>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-white mx-auto">
